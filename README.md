...
首次启动时会在程序同目录自动生成 `config.json`（程序目录不是当前工作目录）：
...
- 默认只监听 **`127.0.0.1`**，局域网内其它机器访问不到
- 默认 API Key 是 **`123456`**，启动后直接改 `config.json` 即可（改完重启生效）
- ⚠️ 如果把 `host` 改成 `0.0.0.0` 对外暴露，**务必同时把 `api_key` 换成足够长的随机串** —— 这个 key 是唯一的访问凭证，拿到它就能消耗你账号的积分
